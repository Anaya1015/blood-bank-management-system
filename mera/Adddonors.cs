...
namespace project
{
    public partial class Adddonors : Form
    {
        ...
        // Function to add a donor to the database
        private void AddDonorToDatabase(string connectionString, string email, string password, string address, string contact)
        {
            ...
            using (SqlConnection connection = new SqlConnection(connectionString))
            {
                using (SqlCommand command = new SqlCommand(query, connection))
                {
                    ...
                    command.Parameters.AddWithValue("@Role", "donor");  // Set role as 'donor'
                    ...
                    try
                    {
                        ...
                        {
                            MessageBox.Show("Failed to add the donor.", "Error");
                        }
                    }
                    catch (Exception ex)
                    {
        ...
        // Optional: Add event handlers for other controls if needed
        private void mailadddonorlbl_Paint(object sender, PaintEventArgs e)
        {
        }

        private void maildonortxt_TextChanged(object sender, EventArgs e)
        {
        }
